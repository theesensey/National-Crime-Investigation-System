...
session_start();
    ...
    //Checking if user is logged in //Contains information about variables passed through a session.
    if(!$_SESSION['user'])
{
...
if ($result) { 
    header("refresh:80;url=../genderhead/viewcrimes.php"); // Refreshes after 20 seconds and Redirects
    echo "<script>
...
<script>
    ...
    form.addEventListener("submit", function(event) {
        ...
        if (result == false) {
          ...
          //Prevent the default action of the form(by default a form submits so we want to disable that)
